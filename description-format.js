(() => {
  const baseDetailMarkup = detailMarkup;

  const colonLabelPattern = /^(Warrior Stance|Tank Stance|Warrior(?:\s+—\s+[^:]+)?|Tank(?:\s+—\s+[^:]+)?|Sun Stance|Moon Stance|Sun Aura|Moon Aura|Sun|Moon|Ally|Enemy|Bright Star|Full Moon|Manifest|Weave|Tick|Tack):\s*(.+)$/i;
  ...
  function formatConditionalDescription(text){
    ...
    const variants = labelled.map((entry, position) => {
      ...
      const continuation = position < labelled.length - 1
        ? sentences.slice(entry.index + 1, nextIndex)
        : [];
      return {
    ...
    return [
      ...
      `<div class="effect-variants">${variants.map(variant => `
        <p class="effect-variant">
          <span class="effect-label">${escapeHtml(variant.label)}</span>
          <span>${escapeHtml(variant.text)}</span>
        </p>`).join('')}</div>`,
      outro.length ? `<p class="effect-common effect-common-after">${escapeHtml(outro.join(' '))}</p>` : ''
    ].join('');
  }

  detailMarkup = function(item, isCore){
    const original = baseDetailMarkup(item, isCore);
    const escapedTooltip = escapeHtml(item.tooltip || '');
    const oldBlock = `<div class="detail-tooltip">${escapedTooltip}</div>`;
    const newBlock = `<div class="detail-tooltip conditional-description">${formatConditionalDescription(item.tooltip || '')}</div>`;
    return original.replace(oldBlock, newBlock);
  };
})();
