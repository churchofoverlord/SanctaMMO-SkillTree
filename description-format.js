const colonLabelPattern =
  /^(Warrior Stance|Tank Stance|Warrior(?:\s+—\s+[^:]+)?|Tank(?:\s+—\s+[^:]+)?|Sun Stance|Moon Stance|Sun Aura|Moon Aura|Sun|Moon|Ally|Enemy|Bright Star|Full Moon|Manifest|Weave|Tick|Tack|Clarity|Surge|Strain|Ice|Lightning|(?:Fire|Ice|Lightning) \+ (?:Fire|Ice|Lightning)):\s*(.+)$/i;
