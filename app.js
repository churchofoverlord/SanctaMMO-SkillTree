...
function renderTree(){
  ...
  for(let tier=1;tier<=4;tier++){
    ...
    head.innerHTML = `
      <div class="tier-title"><span>Tier ${toRoman(tier)}${unlocked ? '' : ' <strong class="tier-locked">LOCKED</strong>'}</span><b>${unlocked ? 'OPEN' : `+${Math.max(0, threshold(tier) - spent())} SP to unlock`}</b></div>
